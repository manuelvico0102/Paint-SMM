...
/**
 * Esta clase define objetos de Ellipse2D a la cual se le agrega un método
 * para facilitar mover los objetos.
 * 
 * @author Manuel Vico
 */
public class MiElipse2D extends Ellipse2D.Float implements MiShape{
    /**
    ...
    /**
     * Constructor para crear e inicializar un objeto MiElipse2D donde se 
     * especifican las coordenadas, así como su ancho y su altura.
     * 
     * @param x valor de la coordena X de la esquina superior izquierda del rectangulo que contiene la elipse
     * @param y valor de la coordena X de la esquina superior izquierda del rectangulo que contiene la elipse
     * @param w el ancho de la elipse
     * @param h la altura de la elipse
     */
    public MiElipse2D(float x, float y, float w, float h){
        super(x, y, w, h);
        this.p1 = new Point2D.Double(x, y);
    }
    
    /**
     * Método para posicionar el objeto MiElipse2D en la posicion dada
     * 
     * @param pos el punto de coordenadas donde posicionar la elipse
     */
